...
class ChatController {
    ...
    sendMessage = async (req: Request, res: Response) => {
        const senderRole: 'user' | 'staff' = req.body.role;
        const message = await chatService.handleIncomingMessage(req.body, senderRole);
        res.status(201).json(message);
    };

  getMessages = async (req: Request, res: Response) => {
        const messages = await chatService.getMessages(req.params.threadId);
        res.json(messages); 
    };
}
